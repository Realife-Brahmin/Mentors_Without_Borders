...
"""
    isWordValid(word::String, validOrNot::Dict, wordsDict::Dict) -> Tuple{Bool, Dict}

Determines if a word is "valid" according to a specific criterion: the word remains valid English as letters are removed one at a time. Validity is defined recursively, with a word considered valid if it's in `wordsDict` and if removing any single letter results in another word that is also valid. The function updates and returns a `validOrNot` dictionary mapping words to their validity status.

# Arguments
- `word::String`: The word to check for validity.
- `validOrNot::Dict`: A dictionary where keys are words and values are Booleans indicating their validity. This dictionary is updated in-place.
- `wordsDict::Dict`: A dictionary of "real" words used as a reference to determine if a word or its "child" words exist.

# Returns
- A tuple containing a Boolean indicating the validity of the input word and the updated `validOrNot` dictionary.

# Notes
- The function employs a recursive approach, initially checking if the word's validity is already recorded in `validOrNot` to avoid redundant computations.
- If the word's validity is not pre-recorded, the function checks its existence in `wordsDict` and then recursively evaluates the validity of all "child" words generated by `generateChildrenWords`.
- A word is marked valid if it exists in `wordsDict` and if it has at least one valid "child" word. Otherwise, it is marked invalid.
- This recursive approach allows for the efficient evaluation of word validity based on the defined criterion, facilitating the exploration of words like the one described in the problem statement (Exercise 12-5 from Think Julia).

# Example
Assuming `validOrNot` and `wordsDict` are pre-populated dictionaries:
```julia
word = "sprite"
validOrNot = Dict{String, Bool}()
wordsDict = Dict("sprite" => true, "spite" => true, "spit" => true, "pit" => true, "it" => true, "i" => true)
validity, updatedValidOrNot = isWordValid(word, validOrNot, wordsDict)

println("Is 'sprite' valid? ", validity)  # Output: Is 'sprite' valid? true
```
"""
...
"""
    validWords(filename::String="words.txt";
            rawDataFolder::String="rawData/",
            extension::String=".txt",
            verbose::Bool=false,
            printResult::Bool=true) -> Dict{String, Bool}

Finds and returns a dictionary of valid words based on a specific validity criterion: a word remains valid as letters are removed one at a time, eventually reducing to one letter, each intermediate form being a valid English word itself. This function reads a word list from a file and evaluates each word for its validity according to the described criterion.

# Arguments
- `filename::String="words.txt"`: Name of the file containing the word list. Defaults to "words.txt".
- `rawDataFolder::String="rawData/"`: Directory where the word list file is located. Defaults to "rawData/".
- `extension::String=".txt"`: File extension of the word list file. Defaults to ".txt".
- `verbose::Bool=false`: If set to `true`, enables verbose output for debugging and insight into the function's process.
- `printResult::Bool=true`: Controls whether the function prints the longest valid word(s) found during its execution.

# Returns
- `Dict{String, Bool}`: A dictionary where the keys are words determined to be valid according to the criterion, and the values are all `true`.

# Notes
- This function builds upon `isWordValid` to check each word in the provided word list for validity.
- Single-letter words "a" and "i", and the empty string "", are explicitly added to `wordsDict` to support the recursive algorithm's base case.
- `validOrNot` is a dictionary that caches the validity status of each word to avoid redundant checks.
- The function identifies the longest valid word(s) as part of its operation if `printResult` is enabled.

# Example
Find and print the longest valid word(s) from a word list in "words.txt":
```julia
validDict = validWords("words.txt", "rawData/", ".txt", verbose=true, printResult=true)
```
"""
function validWords(filename::String="words.txt";
    ...
    for word in keys(wordsDict)
        ...
        printOrNot = verbose && validity 
        myprintln(printOrNot, "Found a valid word: $(word)")
    ...
    if printResult
        ...
        myprintln(printResult, "Longest valid word(s) is(are) $(longestLength) letters long.\n" * "Printing the word(s):")
